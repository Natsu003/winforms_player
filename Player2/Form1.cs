...
namespace Player2
{
    public partial class Form1 : Form
    {
        ...
        {
            _backColor = light ? backLight : backDark;
            _foreColor = light ? foreLight : foreDark;

            // фон
            BackColor = _backColor;
            panelControls.BackColor = _backColor;
            panelPlaylist.BackColor = _backColor;

            // TrackBar-и
            trackBarProgress.BackColor = _backColor;
            trackBarVolume.BackColor = _backColor;

            // список і тексти
            listBoxPlaylistTracks.BackColor = _backColor;
            listBoxPlaylistTracks.ForeColor = _foreColor;
            labelSongInfo.ForeColor = _foreColor;
            labelPlaylistName.ForeColor = _foreColor;
            labelTime.ForeColor = _foreColor;

            // кнопки нижні
            foreach (var b in panelControls.Controls.OfType<Button>())
            { b.BackColor = _backColor; b.ForeColor = _foreColor; }
            // кнопки плейлистів
        ...
        public Form1()
        {
            ...
            // дефолтна обкладинка до першого треку
            ...
            panelPlaylist.Visible = true;  // завжди показуємо панель
        ...
        {
            isShuffle = !isShuffle;
            buttonShuffle.ForeColor = isShuffle ? Color.LimeGreen : _foreColor;
        ...
        {
            isRepeat = !isRepeat;
            buttonRepeat.ForeColor = isRepeat ? Color.LimeGreen : _foreColor;
        }
        ...
        private void cmsPlaylistMenu_ItemClicked_1(object s, ToolStripItemClickedEventArgs e)
        {
            ...
            else if (i == tsmiRenamePlaylist && currentPlaylist != null) ShowNewPlaylistDialog(rename: true);
        }

        // Модальне створення/перейменування плейлисту
        private void ShowNewPlaylistDialog(bool rename = false)
        {
            string current = rename && currentPlaylist != null
                ? currentPlaylist.Name
                : "";

            using var dlg = new NewPlaylistForm(current);
            if (dlg.ShowDialog(this) != DialogResult.OK) return;

            if (rename && currentPlaylist != null)
                currentPlaylist.Name = dlg.PlaylistName;
            else
                Playlists.Add(new Playlist { Name = dlg.PlaylistName });
        ...
        private void RefreshPlaylistButtons()
        {
            ...
            addBtn.Click += (s, e) => ShowNewPlaylistDialog();
